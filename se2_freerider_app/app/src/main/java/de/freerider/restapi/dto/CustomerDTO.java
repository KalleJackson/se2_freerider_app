...
//		return createValidated();
	...
	private Boolean ValidTimeSent() {
		
		if(this.timeSent.getTime() >= 1609459200000l && new Date().getTime() >= this.timeSent.getTime()) {
			return true;
		}
		return false;
	}
	...
	private Optional<Customer> create_() {
		...
		if(!ValidId()) {
			validation.add("id invalid");
		}
		if(this.serial <= 0) {
			validation.add("serial invalid");
		}
		if(this.uuid <= 0) {
			validation.add("uuid invalid");
		}
		if(!ValidTimeSent()) {
			validation.add("timeSent invalid");
		}
		if(!ValidName()) {
			validation.add("name invalid");
		}
